...
int merge(int arr[], int temp[], int left, int mid,int right)
{
    ...
    while ((i <= mid - 1) && (j <= right)) {
        if (arr[i] <= arr[j]) {
            ...
            /* if an element in left Subarray is gretaer than an ele in right< all elements to the right of the left subarray index including it are greater than the right sa element(arrays are sorted) */
